...
class KnownToken(Enum):
    ...
    <p>Tokens to denote well-known (i.e. documented)           versions,
    features, or implementation-defined behaviors,           of XSD.</p>
    ...
    validation episode, at most one keyword in any subtype           will
    apply.  For examples, see the various subtypes           defined
...
class KnownXsdVersion(Enum):
    ...
    <p>           Tokens to denote specific known versions of XSD.
    </p>   <p>           Each token denotes the version of the XSD language
    identified by the <tt>ts:standard-version-id</tt>           attribute on
    the <tt>xsd:enumeration</tt> element.           That is, "<tt>1.0</tt>"
    denotes XSD 1.0 (without reference           to a particular edition),
    and "<tt>1.1</tt>" denotes XSD 1.1           (without referece to a
    particular edition).         </p> </div>
    """
    VALUE_1_0 = "1.0"
    VALUE_1_1 = "1.1"
...
class UnicodeVersions(Enum):
    ...
    <p>           Tokens to denote specific known versions of Unicode.
    </p>   <p>           Each token denotes the version of the Unicode
    specification. The list           is not complete; in the only cases
    where results are known to vary           between Unicode versions,
    results are published for version 4.0.0           and 6.0.0.
    Implementors wishing to provide reference results for           other
    versions of Unicode are welcome to submit such results.         </p>
    </div>
    """
    UNICODE_4_0_0 = "Unicode_4.0.0"
    UNICODE_6_0_0 = "Unicode_6.0.0"
...
class Annotation:
    ...
    <p>           This is an exact copy of the <tt>annotation</tt> element
    defined in the Schema           Recommendation. It is duplicated here in
    order to replicate the           functionality of the
    <tt>xsd:annotation</tt> element and because the Schema for
    Schemas cannot be imported.         </p> </div>
    """
    class Meta:
        name = "annotation"
        namespace = "http://www.w3.org/XML/2004/xml-schema-test-suite/"
...
class DocumentationReference(Ref):
    ...
    <p>           A link to documentation relevant to a test, such as a
    link to the           Recommendation, an erratum, an archived email
    discussion, etc.         </p> </div>
    """
    class Meta:
