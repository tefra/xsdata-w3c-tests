from output.models.nist_data.list_pkg.nmtoken.schema_instance.nistschema_sv_iv_list_nmtoken_pattern_2_xsd.nistschema_sv_iv_list_nmtoken_pattern_2 import NistschemaSvIvListNmtokenPattern2


obj = NistschemaSvIvListNmtokenPattern2(
    value=[
        "NSRL-standards:neede",
        "Internet:computing:The.security-enabling:The.A-will_no:docum",
        "industry_process.computing:popular:recognition:",
        "The:XML-clean:Pervasiv",
        "are.software_which-of:issues.desktop:file-",
        "and:of.Informa",
    ]
)
