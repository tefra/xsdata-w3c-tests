...
def test_xsd001_xsd001_v00(mode, save_output, output_format):
    ...
    simpleContent, and restriction. - simpleType within restriction.
    """
    assert_bindings(
...
def test_test003b_test_1_v(mode, save_output, output_format):
    """
    A variant of test003 that blocks extension on elements but not on
    complex types Produced by Michael Kay, Saxonica, August 2023, in
    response to Saxon bug 6153
    """
    assert_bindings(
        schema="sunData/combined/003a/test.xsd",
        instance="sunData/combined/003a/test.1.v.xml",
        class_name="Root",
        version="1.1",
        mode=mode,
        save_output=save_output,
...
def test_test003a_test_1_v(mode, save_output, output_format):
    """
    A variant of test003 that blocks extension on complex types but not on
    elements Produced by Michael Kay, Saxonica, August 2023, in response
    to Saxon bug 6153
    """
    assert_bindings(
        schema="sunData/combined/003a/test.xsd",
...
def test_annotation00101m4_positive(mode, save_output, output_format):
    ...
    annotations of schema components. In the test the machine-targeted
    annotation is provided for an attribute group definition.
    """
    assert_bindings(
...
def test_annotation00101m1_positive(mode, save_output, output_format):
    ...
    schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the human-targeted
    annotation is provided for an attribute group definition.
    """
    assert_bindings(
...
def test_ag_attrusens00101m1_p_positive(mode, save_output, output_format):
    ...
    ref value to be resolved to its declaration.
    """
    assert_bindings(
...
def test_ad_valconstr00201m3_positive(mode, save_output, output_format):
    ...
    'use' are both present, 'use' must have the actual value 'optional'.
    """
    assert_bindings(
...
def test_ad_name00118_ad_name00118_p(mode, save_output, output_format):
    ...
    type int. Name them using Unicode letter #x01BB which is neither upper
    nor lower. The document AD_name00118_p.xml sets all the attributes.
    """
    assert_bindings(
...
def test_ad_name00115_ad_name00115_p(mode, save_output, output_format):
    ...
    first one as "_-." and the second one as "_-0.". The document
    AD_name00115_p.xml sets the attributes to 0 and 1 respectively.
    """
    assert_bindings(
...
def test_ad_name00114_ad_name00114_p(mode, save_output, output_format):
    ...
    attributes of type int. Name them as follows: "aaaa", "bbbB", "ccCc",
    "ddDD", "eEee", "fFfF", "pPPp", "gGGG", "Hhhh", "IiiI", "JjJj",
    "KkKK", "LLll", "MMmM", "NNNn", "OOOO", "bbb0", "cc0c", "dd00",
    "e0ee", "f0f0", "p00p", "g000", "bbb_", "cc_c", "dd__", "e_ee",
    "f_f_", "p__p", "g___", "H111", "I11I", "J1J1", "K1KK", "LL11",
    "MM1M", "NNN1", "H---", "I--I", "J-J-", "K-KK", "LL--", "MM-M",
    "NNN-". The document AD_name00114_p.xml sets all the attributes.
    """
    assert_bindings(
...
def test_ad_name00113_ad_name00113_p(mode, save_output, output_format):
    ...
    Name them as follows: "aa111a2Aa", "aa22B3c", "aa3-4_". The document
    AD_name00113_p.xml sets the attributes to 0, 1 and 2 respectively.
    """
    assert_bindings(
...
def test_ad_name00112_ad_name00112_p(mode, save_output, output_format):
    ...
    using 7 punctuation characters: hyphen ('-', \u002D, HYPHEN-MINUS),
    period ('.', \u002E, FULL STOP), underscore ('_', \u005F, LOW LINE),
...
def test_ad_name00111_ad_name00111_p(mode, save_output, output_format):
    ...
    attributes using 7 punctuation characters: hyphen ('-', \u002D,
    HYPHEN-MINUS), period ('.', \u002E, FULL STOP), underscore ('_',
    \u005F, LOW LINE), dot ('.', \u00B7, MIDDLE DOT), \u0387, GREEK ANO
    TELEIA, \u06DD, ARABIC END OF AYAH and \u06DE, ARABIC START OF RUB EL
    HIZB. The document AD_name00111_p.xml sets the attributes to 0, 1, 2,
    ..., 6 respectively.
    """
    assert_bindings(
...
def test_ad_name00110m9_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following combining
    characters: 0x0f39, 0x0f3e, 0x0f3f, 0x0f71, 0x0f7a, 0x0f84, 0x0f86,
    0x0f86, 0x0f87, 0x0f90, 0x0f92, 0x0f95, 0x0f97, 0x0f99, 0x0fa3,
    0x0fad, 0x0fb1, 0x0fb4, 0x0fb7, 0x0fb9 respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m8_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following combining
    characters: 0x0e31, 0x0e34, 0x0e37, 0x0e3a, 0x0e47, 0x0e4a, 0x0e4e,
    0x0eb1, 0x0eb4, 0x0eb6, 0x0eb9, 0x0ebb, 0x0ebb, 0x0ebc, 0x0ec8,
    0x0eca, 0x0ecd, 0x0f18, 0x0f18, 0x0f19, 0x0f35, 0x0f37 respectively.
...
def test_ad_name00110m7_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following combining
    characters: 0x0c82, 0x0c82, 0x0c83, 0x0cbe, 0x0cc1, 0x0cc4, 0x0cc6,
    0x0cc7, 0x0cc8, 0x0cca, 0x0ccb, 0x0ccd, 0x0cd5, 0x0cd5, 0x0cd6,
    0x0d02, 0x0d02, 0x0d03, 0x0d3e, 0x0d40, 0x0d43, 0x0d46, 0x0d47,
    0x0d48, 0x0d4a, 0x0d4b, 0x0d4d, 0x0d57 respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m6_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following combining
    characters: 0x0b82, 0x0b82, 0x0b83, 0x0bbe, 0x0bc0, 0x0bc2, 0x0bc6,
    0x0bc7, 0x0bc8, 0x0bca, 0x0bcb, 0x0bcd, 0x0bd7, 0x0c01, 0x0c02,
    0x0c03, 0x0c3e, 0x0c41, 0x0c44, 0x0c46, 0x0c47, 0x0c48, 0x0c4a,
    0x0c4b, 0x0c4d, 0x0c55, 0x0c55, 0x0c56 respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m5_positive(mode, save_output, output_format):
    ...
    end with the following combining characters: 0x0abc, 0x0abe, 0x0ac1,
    0x0ac5, 0x0ac7, 0x0ac8, 0x0ac9, 0x0acb, 0x0acc, 0x0acd, 0x0b01,
    0x0b02, 0x0b03, 0x0b3c, 0x0b3e, 0x0b40, 0x0b43, 0x0b47, 0x0b47,
    0x0b48, 0x0b4b, 0x0b4c, 0x0b4d, 0x0b56, 0x0b56, 0x0b57 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m4_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following combining
    characters: 0x09e2, 0x09e2, 0x09e3, 0x0a02, 0x0a3c, 0x0a3e, 0x0a3f,
    0x0a40, 0x0a41, 0x0a42, 0x0a47, 0x0a47, 0x0a48, 0x0a4b, 0x0a4c,
    0x0a4d, 0x0a70, 0x0a70, 0x0a71, 0x0a81, 0x0a82, 0x0a83 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m3_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following combining
    characters: 0x0951, 0x0952, 0x0954, 0x0962, 0x0962, 0x0963, 0x0981,
    0x0982, 0x0983, 0x09bc, 0x09be, 0x09bf, 0x09c0, 0x09c2, 0x09c4,
    0x09c7, 0x09c7, 0x09c8, 0x09cb, 0x09cc, 0x09cd, 0x09d7 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m2_positive(mode, save_output, output_format):
    ...
    following combining characters: 0x0670, 0x06d6, 0x06d9, 0x06dc,
    0x06dd, 0x06de, 0x06df, 0x06e0, 0x06e2, 0x06e4, 0x06e7, 0x06e7,
    0x06e8, 0x06ea, 0x06eb, 0x06ed, 0x0901, 0x0902, 0x0903, 0x093c,
    0x093e, 0x0945, 0x094c, 0x094d respectively. The document set their
    values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m10_positive(mode, save_output, output_format):
    ...
    the following combining characters: 0x20d0, 0x20d6, 0x20dc, 0x20e1,
    0x302a, 0x302c, 0x302f, 0x3099, 0x309a respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00110m1_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare 26 local string attributes with the names that
    end with the following combining characters: 0x0300, 0x0322, 0x0344,
    0x0360, 0x0360, 0x0361, 0x0483, 0x0484, 0x0486, 0x0591, 0x0599,
    0x05a1, 0x05a3, 0x05ae, 0x05b9, 0x05bb, 0x05bc, 0x05bd, 0x05bf,
    0x05c1, 0x05c1, 0x05c2, 0x05c4, 0x064b, 0x064e, 0x0652 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00109m1_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following digit
    characters: 0x0030, 0x0034, 0x0039, 0x0660, 0x0664, 0x0669, 0x06f0,
    0x06f4, 0x06f9, 0x0966, 0x096a, 0x096f, 0x09e6, 0x09ea, 0x09ef,
    0x0a66, 0x0a6a, 0x0a6f, 0x0ae6, 0x0aea, 0x0aef, 0x0b66, 0x0b6a,
    0x0b6f, 0x0be7, 0x0beb, 0x0bef, 0x0c66, 0x0c6a, 0x0c6f respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m9_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare 26 local string attributes with the names that
    end with the following basic characters: 0x0a8f, 0x0a90, 0x0a91,
    0x0a93, 0x0a9d, 0x0aa8, 0x0aaa, 0x0aad, 0x0ab0, 0x0ab2, 0x0ab2,
    0x0ab3, 0x0ab5, 0x0ab7, 0x0ab9, 0x0abd, 0x0ae0, 0x0b05, 0x0b08,
    0x0b0c, 0x0b0f, 0x0b0f, 0x0b10, 0x0b13, 0x0b1d, 0x0b28 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m8_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare 26 local string attributes with the names that
    end with the following basic characters: 0x0a13, 0x0a1d, 0x0a28,
    0x0a2a, 0x0a2d, 0x0a30, 0x0a32, 0x0a32, 0x0a33, 0x0a35, 0x0a35,
    0x0a36, 0x0a38, 0x0a38, 0x0a39, 0x0a59, 0x0a5a, 0x0a5c, 0x0a5e,
    0x0a72, 0x0a73, 0x0a74, 0x0a85, 0x0a88, 0x0a8b, 0x0a8d respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m7_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x098f, 0x098f, 0x0990, 0x0993, 0x099d, 0x09a8, 0x09aa,
    0x09ad, 0x09b0, 0x09b2, 0x09b6, 0x09b7, 0x09b9, 0x09dc, 0x09dc,
    0x09dd, 0x09df, 0x09e0, 0x09e1, 0x09f0, 0x09f0, 0x09f1, 0x0a05,
    0x0a07, 0x0a0a, 0x0a0f, 0x0a0f, 0x0a10 respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m6_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare 26 local string attributes with the names that
    end with the following basic characters: 0x0671, 0x0694, 0x06b7,
    0x06ba, 0x06bc, 0x06be, 0x06c0, 0x06c7, 0x06ce, 0x06d0, 0x06d1,
    0x06d3, 0x06d5, 0x06e5, 0x06e5, 0x06e6, 0x0905, 0x091f, 0x0939,
    0x093d, 0x0958, 0x095c, 0x0961, 0x0985, 0x0988, 0x098c respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m5_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x04d0, 0x04dd, 0x04eb, 0x04ee, 0x04f1, 0x04f5, 0x04f8,
    0x04f8, 0x04f9, 0x0531, 0x0543, 0x0556, 0x0559, 0x0561, 0x0573,
    0x0586, 0x05d0, 0x05dd, 0x05ea, 0x05f0, 0x05f1, 0x05f2, 0x0621,
    0x062d, 0x063a, 0x0641, 0x0645, 0x064a respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m4_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following basic
    characters: 0x03d0, 0x03d3, 0x03d6, 0x03e2, 0x03e9, 0x03f1, 0x0401,
    0x0406, 0x040c, 0x040e, 0x042e, 0x044f, 0x0451, 0x0456, 0x045c,
    0x045e, 0x046f, 0x0481, 0x0490, 0x04a7, 0x04bf, 0x04c1, 0x04c2,
    0x04c4, 0x04c7, 0x04c7, 0x04c8, 0x04cb, 0x04cb, 0x04cc respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m3_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare 26 local string attributes with the names that
    end with the following basic characters: 0x0276, 0x027a, 0x027f,
    0x0281, 0x0294, 0x02a8, 0x02bb, 0x02be, 0x02c1, 0x0386, 0x0388,
    0x0389, 0x038a, 0x038c, 0x038e, 0x038e, 0x038f, 0x0391, 0x0399,
    0x03a1, 0x03a3, 0x03a9, 0x03af, 0x03b1, 0x03bf, 0x03ce respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m21_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x1fe8, 0x1fea, 0x1fec, 0x1ff8, 0x1ff9, 0x1ffb, 0x2180,
    0x2181, 0x2182, 0x3041, 0x306a, 0x3094, 0x30a1, 0x30cd, 0x30fa,
    0x3105, 0x3118, 0x312c, 0xac00, 0xc1d1, 0xd7a3 respectively. The
...
def test_ad_name00108m20_positive(mode, save_output, output_format):
    ...
    Declare 24 local string attributes with the names that end with the
    following basic characters: 0x1f5b, 0x1f5d, 0x1f5f, 0x1f6e, 0x1f7d,
    0x1fb0, 0x1fb0, 0x1fb1, 0x1fb8, 0x1fb9, 0x1fbb, 0x1fc8, 0x1fc9,
    0x1fcb, 0x1fd0, 0x1fd0, 0x1fd1, 0x1fd8, 0x1fd9, 0x1fdb, 0x1fe0,
    0x1fe0, 0x1fe1, 0x1fe5 respectively. The document set their values to
    0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m2_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following basic
    characters: 0x014a, 0x0164, 0x017e, 0x0180, 0x018a, 0x0194, 0x0196,
    0x019d, 0x01a5, 0x01a7, 0x01a8, 0x01a9, 0x01ab, 0x01b4, 0x01bd,
    0x01c0, 0x01c1, 0x01c3, 0x01cd, 0x01de, 0x01ef, 0x01f4, 0x01f4,
    0x01f5, 0x01fa, 0x0208, 0x0217, 0x0250, 0x0262, 0x0274 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m19_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x1ea0, 0x1ecc, 0x1ef9, 0x1f00, 0x1f0a, 0x1f15, 0x1f18,
    0x1f1a, 0x1f1d, 0x1f20, 0x1f32, 0x1f45, 0x1f48, 0x1f4a, 0x1f4d,
    0x1f51, 0x1f53, 0x1f55, 0x1f57, 0x1f59 respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m18_positive(mode, save_output, output_format):
    ...
    end with the following basic characters: 0x11ab, 0x11ae, 0x11ae,
    0x11af, 0x11b7, 0x11b7, 0x11b8, 0x11ba, 0x11bc, 0x11bf, 0x11c2,
    0x11eb, 0x11f0, 0x11f9, 0x1e00, 0x1e4a, 0x1e95, 0x1e9b respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m17_positive(mode, save_output, output_format):
    ...
    following basic characters: 0x115f, 0x1160, 0x1161, 0x1163, 0x1165,
    0x1167, 0x1169, 0x116d, 0x116d, 0x116e, 0x1172, 0x1172, 0x1173,
    0x1175, 0x119e, 0x11a8 respectively. The document set their values to
    0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m16_positive(mode, save_output, output_format):
    ...
    Declare 16 local string attributes with the names that end with the
    following basic characters: 0x110b, 0x110b, 0x110c, 0x110e, 0x1110,
    0x1112, 0x113c, 0x113e, 0x1140, 0x114c, 0x114e, 0x1150, 0x1154,
    0x1154, 0x1155, 0x1159 respectively. The document set their values to
    0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m15_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following basic
    characters: 0x0eb0, 0x0eb2, 0x0eb2, 0x0eb3, 0x0ebd, 0x0ec0, 0x0ec2,
    0x0ec4, 0x0f40, 0x0f43, 0x0f47, 0x0f49, 0x0f59, 0x0f69, 0x1100,
    0x1102, 0x1102, 0x1103, 0x1105, 0x1106, 0x1107, 0x1109 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m14_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following basic
    characters: 0x0e87, 0x0e87, 0x0e88, 0x0e8a, 0x0e8d, 0x0e94, 0x0e95,
    0x0e97, 0x0e99, 0x0e9c, 0x0e9f, 0x0ea1, 0x0ea2, 0x0ea3, 0x0ea5,
    0x0ea7, 0x0eaa, 0x0eaa, 0x0eab, 0x0ead, 0x0ead, 0x0eae respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m13_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare 26 local string attributes with the names that
    end with the following basic characters: 0x0d0e, 0x0d0f, 0x0d10,
    0x0d12, 0x0d1d, 0x0d28, 0x0d2a, 0x0d31, 0x0d39, 0x0d60, 0x0d60,
    0x0d61, 0x0e01, 0x0e17, 0x0e2e, 0x0e30, 0x0e32, 0x0e32, 0x0e33,
    0x0e40, 0x0e42, 0x0e45, 0x0e81, 0x0e81, 0x0e82, 0x0e84 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m12_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x0c35, 0x0c37, 0x0c39, 0x0c60, 0x0c60, 0x0c61, 0x0c85,
    0x0c88, 0x0c8c, 0x0c8e, 0x0c8f, 0x0c90, 0x0c92, 0x0c9d, 0x0ca8,
    0x0caa, 0x0cae, 0x0cb3, 0x0cb5, 0x0cb7, 0x0cb9, 0x0cde, 0x0ce0,
    0x0ce0, 0x0ce1, 0x0d05, 0x0d08, 0x0d0c respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m11_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x0b9c, 0x0b9e, 0x0b9e, 0x0b9f, 0x0ba3, 0x0ba3, 0x0ba4,
    0x0ba8, 0x0ba9, 0x0baa, 0x0bae, 0x0bb1, 0x0bb5, 0x0bb7, 0x0bb8,
    0x0bb9, 0x0c05, 0x0c08, 0x0c0c, 0x0c0e, 0x0c0f, 0x0c10, 0x0c12,
    0x0c1d, 0x0c28, 0x0c2a, 0x0c2e, 0x0c33 respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m10_positive(mode, save_output, output_format):
    ...
    attributes with the names that end with the following basic
    characters: 0x0b2a, 0x0b2d, 0x0b30, 0x0b32, 0x0b32, 0x0b33, 0x0b36,
    0x0b37, 0x0b39, 0x0b3d, 0x0b5c, 0x0b5c, 0x0b5d, 0x0b5f, 0x0b60,
    0x0b61, 0x0b85, 0x0b87, 0x0b8a, 0x0b8e, 0x0b8f, 0x0b90, 0x0b92,
    0x0b93, 0x0b95, 0x0b99, 0x0b99, 0x0b9a respectively. The document set
    their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00108m1_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following basic
    characters: 0x0041, 0x004d, 0x005a, 0x0061, 0x0064, 0x0068, 0x006a,
    0x0072, 0x007a, 0x00c0, 0x00cb, 0x00d6, 0x00d8, 0x00db, 0x00de,
    0x00e0, 0x00eb, 0x00f6, 0x00f8, 0x00fb, 0x00ff, 0x0100, 0x0118,
    0x0131, 0x0134, 0x0139, 0x013e, 0x0141, 0x0144, 0x0148 respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00105m1_positive(mode, save_output, output_format):
    ...
    begins with the underscore character 0x005f. The document set the
...
def test_ad_name00104m9_positive(mode, save_output, output_format):
    ...
    begin with the following basic characters: 0x0a8f, 0x0a90, 0x0a91,
    0x0a93, 0x0a9d, 0x0aa8, 0x0aaa, 0x0aad, 0x0ab0, 0x0ab2, 0x0ab2,
    0x0ab3, 0x0ab5, 0x0ab7, 0x0ab9, 0x0abd, 0x0ae0, 0x0b05, 0x0b08,
    0x0b0c, 0x0b0f, 0x0b0f, 0x0b10, 0x0b13, 0x0b1d, 0x0b28 respectively.
    The document set their values to 0, 1 or 2.
...
def test_ad_name00104m8_positive(mode, save_output, output_format):
    ...
    0x0a5c, 0x0a5e, 0x0a72, 0x0a73, 0x0a74, 0x0a85, 0x0a88, 0x0a8b, 0x0a8d
    (valid schema) Declare 26 local integer attributes with the names that
    begin with the following basic characters: 0x0a13, 0x0a1d, 0x0a28,
    0x0a2a, 0x0a2d, 0x0a30, 0x0a32, 0x0a32, 0x0a33, 0x0a35, 0x0a35,
    0x0a36, 0x0a38, 0x0a38, 0x0a39, 0x0a59, 0x0a5a, 0x0a5c, 0x0a5e,
    0x0a72, 0x0a73, 0x0a74, 0x0a85, 0x0a88, 0x0a8b, 0x0a8d respectively.
    The document set their values to 0, 1 or 2.
...
def test_ad_name00104m7_positive(mode, save_output, output_format):
    ...
    attributes with the names that begin with the following basic
    characters: 0x098f, 0x098f, 0x0990, 0x0993, 0x099d, 0x09a8, 0x09aa,
    0x09ad, 0x09b0, 0x09b2, 0x09b6, 0x09b7, 0x09b9, 0x09dc, 0x09dc,
    0x09dd, 0x09df, 0x09e0, 0x09e1, 0x09f0, 0x09f0, 0x09f1, 0x0a05,
    0x0a07, 0x0a0a, 0x0a0f, 0x0a0f, 0x0a10 respectively. The document set
...
def test_ad_name00104m6_positive(mode, save_output, output_format):
    ...
    0x091f, 0x0939, 0x093d, 0x0958, 0x095c, 0x0961, 0x0985, 0x0988, 0x098c
    (valid schema) Declare 26 local integer attributes with the names that
    begin with the following basic characters: 0x0671, 0x0694, 0x06b7,
    0x06ba, 0x06bc, 0x06be, 0x06c0, 0x06c7, 0x06ce, 0x06d0, 0x06d1,
    0x06d3, 0x06d5, 0x06e5, 0x06e5, 0x06e6, 0x0905, 0x091f, 0x0939,
    0x093d, 0x0958, 0x095c, 0x0961, 0x0985, 0x0988, 0x098c respectively.
    The document set their values to 0, 1 or 2.
...
def test_ad_name00104m4_positive(mode, save_output, output_format):
    ...
    0x0481, 0x0490, 0x04a7, 0x04bf, 0x04c1, 0x04c2, 0x04c4, 0x04c7,
    0x04c7, 0x04c8, 0x04cb, 0x04cb, 0x04cc (valid schema) Declare 30 local
    integer attributes with the names that begin with the following basic
    characters: 0x03d0, 0x03d3, 0x03d6, 0x03e2, 0x03e9, 0x03f1, 0x0401,
    0x0406, 0x040c, 0x040e, 0x042e, 0x044f, 0x0451, 0x0456, 0x045c,
    0x045e, 0x046f, 0x0481, 0x0490, 0x04a7, 0x04bf, 0x04c1, 0x04c2,
    0x04c4, 0x04c7, 0x04c7, 0x04c8, 0x04cb, 0x04cb, 0x04cc respectively.
...
def test_ad_name00104m3_positive(mode, save_output, output_format):
    ...
    0x0391, 0x0399, 0x03a1, 0x03a3, 0x03a9, 0x03af, 0x03b1, 0x03bf, 0x03ce
    (valid schema) Declare 26 local integer attributes with the names that
    begin with the following basic characters: 0x0276, 0x027a, 0x027f,
    0x0281, 0x0294, 0x02a8, 0x02bb, 0x02be, 0x02c1, 0x0386, 0x0388,
    0x0389, 0x038a, 0x038c, 0x038e, 0x038e, 0x038f, 0x0391, 0x0399,
    0x03a1, 0x03a3, 0x03a9, 0x03af, 0x03b1, 0x03bf, 0x03ce respectively.
    The document set their values to 0, 1 or 2.
...
def test_ad_name00104m18_positive(mode, save_output, output_format):
    ...
    begin with the following basic characters: 0x11ab, 0x11ae, 0x11ae,
    0x11af, 0x11b7, 0x11b7, 0x11b8, 0x11ba, 0x11bc, 0x11bf, 0x11c2,
    0x11eb, 0x11f0, 0x11f9, 0x1e00, 0x1e4a, 0x1e95, 0x1e9b respectively.
    The document set their values to 0, 1 or 2.
    """
...
def test_ad_name00104m13_positive(mode, save_output, output_format):
    ...
    0x0e32, 0x0e33, 0x0e40, 0x0e42, 0x0e45, 0x0e81, 0x0e81, 0x0e82, 0x0e84
    (valid schema) Declare 26 local integer attributes with the names that
    begin with the following basic characters: 0x0d0e, 0x0d0f, 0x0d10,
    0x0d12, 0x0d1d, 0x0d28, 0x0d2a, 0x0d31, 0x0d39, 0x0d60, 0x0d60,
    0x0d61, 0x0e01, 0x0e17, 0x0e2e, 0x0e30, 0x0e32, 0x0e32, 0x0e33,
    0x0e40, 0x0e42, 0x0e45, 0x0e81, 0x0e81, 0x0e82, 0x0e84 respectively.
    The document set their values to 0, 1 or 2.
...
def test_ad_name00103m2_positive(mode, save_output, output_format):
    ...
    the names that end with the following extender characters: 0x30fc,
    0x30fd, 0x30fe respectively. The document set their values to 0, 1 or
    2.
    """
    assert_bindings(
...
def test_ad_name00103m1_positive(mode, save_output, output_format):
    ...
    string attributes with the names that end with the following extender
    characters: 0x00b7, 0x02d0, 0x02d1, 0x0387, 0x0640, 0x0e46, 0x0ec6,
    0x3005, 0x3031, 0x3033, 0x3035, 0x309d, 0x309d, 0x309e respectively.
    The document set their values to 0, 1 or 2.
    """
    assert_bindings(
...
def test_ad_name00102m1_positive(mode, save_output, output_format):
    ...
    following ideographic characters: 0x4e00, 0x76d2, 0x9fa5, 0x3007,
    0x3021, 0x3025, 0x3029 respectively. The document set their values to
    0, 1 or 2.
    """
    assert_bindings(
...
def test_psubstitutions00104m_p_substitutions00104m1_p(mode, save_output, output_format):
    ...
    *content model* is prevented from additionally *validating* element
    items with an xsi:type (p.2.6.1) attribute.
    """
    assert_bindings(
...
def test_final00101m3_positive(mode, save_output, output_format):
    ...
    restriction respectively. If all values are specified, then the
    complex type is said to be final, because no further derivations are
    possible.
    """
    assert_bindings(
...
def test_derivationmethod00102m2_positive(mode, save_output, output_format):
    ...
    (Extension). If the {base type definition} is a simple type
    definition.
    """
    assert_bindings(
...
def test_derivationmethod00101m2_positive(mode, save_output, output_format):
    ...
    definition} is a complex type definition.
    """
    assert_bindings(
...
def test_contenttype00401m_content_type00401m1_p(mode, save_output, output_format):
    ...
    *validates* elements whose element [children] (i.e. specifically
    ignoring other [children] such as character information items) conform
    to the supplied *content model*.
    """
    assert_bindings(
...
def test_annotation00101m1_positive_172(mode, save_output, output_format):
    """
...
def test_valueconstraint01101m3_positive(mode, save_output, output_format):
    ...
    simple type definition, then the string must be valid with respect to
    that definition. In the test the default value is valid.
    """
    assert_bindings(
...
def test_valueconstraint01101m1_positive(mode, save_output, output_format):
    ...
    type definition, then the string must be valid with respect to that
    definition. In the test the fixed value is valid.
    """
    assert_bindings(
...
def test_valueconstraint00901m1_positive(mode, save_output, output_format):
    ...
    definition, then the actual value of the item must match the canonical
    lexical representation of the {value constraint} value In the test,
    the value of the item does not match the specified fixed value.
    """
    assert_bindings(
...
def test_valueconstraint00801m1_positive(mode, save_output, output_format):
    ...
    initial value of the item must match the canonical lexical
    representation of the {value constraint} value. In the test, the value
    of the item does not match the specified fixed value.
    """
    assert_bindings(
...
def test_valueconstraint00601m7_positive(mode, save_output, output_format):
    ...
    constraint} value used as its normalized value must be valid with
    respect to the actual type definition . In the test the default value
    a derived type is valid.
    """
    assert_bindings(
...
def test_valueconstraint00601m5_positive(mode, save_output, output_format):
    ...
    information item with the lexical representation of the {value
    constraint} value used as its normalized value must be valid with
    respect to the actual type definition . In the test the fixed value of
    a derived type is valid.
    """
    assert_bindings(
...
def test_valueconstraint00501m5_positive(mode, save_output, output_format):
    ...
    xsi:nil is not specified, the actual type definition is a local type
    definition then the lexical representation of the {value constraint}
    value must be a valid default for the actual type definition . In the
...
def test_valueconstraint00402m7_positive(mode, save_output, output_format):
    ...
    answer allows only "true". Check that the schema is valid.
    """
    assert_bindings(
...
def test_valueconstraint00402m5_positive(mode, save_output, output_format):
    ...
    content type of type answer is boolean . Check that the schema is
    valid.
    """
    assert_bindings(
...
def test_valueconstraint00201m_value_constraint00201m1_p(mode, save_output, output_format):
    ...
    or, otherwise, must be equal to that defined.
    """
    assert_bindings(
...
def test_typedef01501m1_positive(mode, save_output, output_format):
    ...
    information item must be valid with respect to the type definition.
...
def test_typedef01401m1_positive(mode, save_output, output_format):
    ...
    nil, then its normalized value must be valid with respect to the type
    definition. Negative case uses invalid value.
    """
    assert_bindings(
...
def test_typedef01301m1_positive(mode, save_output, output_format):
    ...
    item children if it has a simple type. Negative case uses an element
    of type xsd:anySimpleType with children elements.
    """
    assert_bindings(
...
def test_typedef01202m1_positive(mode, save_output, output_format):
    ...
    definition, then the element information item's attributes must be
    ...
    negative case uses an element with an extra attribute.
    """
    assert_bindings(
...
def test_typedef01201m1_positive(mode, save_output, output_format):
    ...
    then the element information item's attributes must be empty,
...
def test_typedef01101m1_positive(mode, save_output, output_format):
    ...
    type definition with a type that is abstract .
    """
    assert_bindings(
...
def test_typedef00901m1_positive(mode, save_output, output_format):
    ...
    valid with respect to the actual type definition. In the test the
    actual type definition is a local type definition.
    """
    assert_bindings(
...
def test_typedef00802m1_positive(mode, save_output, output_format):
    ...
    {disallowed substitutions}. In the test the local type definition is
    validly derived from simpleType.
    """
    assert_bindings(
...
def test_typedef00801m1_positive(mode, save_output, output_format):
    ...
    the {disallowed substitutions} and the {type definition}'s {prohibited
    substitutions}. In the test the local type definition is validly
    derived from complexType.
    """
    assert_bindings(
...
def test_typedef00701m_type_def00701m1_p(mode, save_output, output_format):
    ...
    has both local and namespace names.
    """
    assert_bindings(
...
def test_typedef00403m_type_def00403m1_p(mode, save_output, output_format):
    ...
    when the declaration can use a built-in or pre-declared type
...
def test_typedef00402m_type_def00402m1_p(mode, save_output, output_format):
    ...
    declarations (if there's a ref attribute) or local declarations
    otherwise.
    """
    assert_bindings(
...
def test_typedef00401m_type_def00401m1_p(mode, save_output, output_format):
    ...
    a ref attribute) or local declarations otherwise.
    """
    assert_bindings(
...
def test_typedef00301m_type_def00301m1_p(mode, save_output, output_format):
    ...
    element. Specify the type by reference. Ensure the value is validated
...
def test_typedef00205m_type_def00205m1_p(mode, save_output, output_format):
    ...
    element of type anyType. Ensure the anyType element may substitute for
    the default type one.
    """
    assert_bindings(
...
def test_typedef00203m_type_def00203m1_p(mode, save_output, output_format):
    ...
    is validated according to the type.
    """
    assert_bindings(
...
def test_typedef00202m_type_def00202m1_p(mode, save_output, output_format):
    ...
    validated according to the type.
    """
    assert_bindings(
...
def test_term00101m_term00101m1_p(mode, save_output, output_format):
    ...
    refers to a global one. Check that the element is validated according
    to the type of the referred element.
    """
    assert_bindings(
...
def test_targetns00402m_target_ns00402m1_p(mode, save_output, output_format):
    ...
    level declaration must be unqualified.
    """
    assert_bindings(
...
def test_targetns00303m3_positive(mode, save_output, output_format):
    ...
    element declaration. Set the elementFormDefault to unqualified .
    """
    assert_bindings(
...
def test_targetns00303m2_positive(mode, save_output, output_format):
    ...
    element declaration to unqualified . Set the elementFormDefault to
    unqualified .
    """
    assert_bindings(
...
def test_targetns00303m1_positive(mode, save_output, output_format):
    ...
    element declaration to qualified . Set the elementFormDefault to
    unqualified .
    """
    assert_bindings(
...
def test_targetns00302m3_positive(mode, save_output, output_format):
    ...
    declaration. Set the elementFormDefault to qualified .
    """
    assert_bindings(
...
def test_targetns00301m2_positive(mode, save_output, output_format):
    ...
    local element declaration to unqualified . Omit the elementFormDefault
    attribute.
    """
    assert_bindings(
...
def test_targetns00301m1_positive(mode, save_output, output_format):
    ...
    attribute is omitted. (valid schema) Set the form attribute of the
    local element declaration to qualified . Omit the elementFormDefault
    attribute.
    """
    assert_bindings(
...
def test_targetns00101m_target_ns00101m1_p_243(mode, save_output, output_format):
    """
    ...
    incompatible. Check that validation takes into account the property
    {target namespace}.
    """
    assert_bindings(
...
def test_substgrpexcl00303m3_positive(mode, save_output, output_format):
    ...
    by another element. The elements have the type extended from the type
    of the head element. Try to rule out both restriction and extension by
    means of {substitution group exclusions} property of the head element
    declaration. Try to substitute the head with the first element.
    """
    assert_bindings(
...
def test_substgrpexcl00301m3_positive(mode, save_output, output_format):
    ...
    Rule out both restriction and extension substitutions (positive case).
    (valid schema) Define one element within a substitution group headed
    by another element. The elements have the same type. Try to rule out
    both restriction and extension by means of {substitution group
    exclusions} property of the head element declaration. Try to
    substitute the head with the first element.
    """
    assert_bindings(
...
def test_substgrpexcl00202m1_positive(mode, save_output, output_format):
    ...
    same type, the second is derived as restriction and the third derived
    as extension. All the elments are the memebers of the substitution
    group. Try to rule out extension by means of {substitution group
    exclusions} property of the head element declaration.
    """
    assert_bindings(
...
def test_substgrpaffil00201m_subst_grp_affil00201m1_p(mode, save_output, output_format):
    ...
    Then the last one within another group. Check the following: 1. the
    first element substitutes the element of the second group 2. the
    second element doesn't substitute the first one
    """
    assert_bindings(
...
def test_nillable00302m_nillable00302m1_p(mode, save_output, output_format):
    ...
    complex type. Set the element to be nil . Negative case tries to have
    children.
    """
    assert_bindings(
...
def test_nillable00301m_nillable00301m1_p(mode, save_output, output_format):
    ...
    to be nil . Negative case tries to have characters.
    """
    assert_bindings(
...
def test_name00805_name00805_p(mode, save_output, output_format):
    ...
    Name the first one as "_-." and the second one as "_-0.". The document
    name00805_p.xml sets the attributes to 0 and 1 respectively.
    """
    assert_bindings(
...
def test_name00804_name00804_p(mode, save_output, output_format):
    ...
    integer elements. Name them as follows: "aaaa", "bbbB", "ccCc",
    "ddDD", "eEee", "fFfF", "pPPp", "gGGG", "Hhhh", "IiiI", "JjJj",
    "KkKK", "LLll", "MMmM", "NNNn", "OOOO", "bbb0", "cc0c", "dd00",
    "e0ee", "f0f0", "p00p", "g000", "bbb_", "cc_c", "dd__", "e_ee",
    "f_f_", "p__p", "g___", "H111", "I11I", "J1J1", "K1KK", "LL11",
    "MM1M", "NNN1", "H---", "I--I", "J-J-", "K-KK", "LL--", "MM-M",
    "NNN-". The document name00804_p.xml sets all the elements.
    """
    assert_bindings(
...
def test_name00803_name00803_p(mode, save_output, output_format):
    ...
    elements. Name them as follows: "aa111a2Aa", "aa22B3c", "aa3-4_". The
    document name00803_p.xml sets the elements to 0, 1 and 2 respectively.
    """
    assert_bindings(
...
def test_name00802_name00802_p(mode, save_output, output_format):
    ...
    dot ('.', \u00B7, MIDDLE DOT), \u0387, GREEK ANO TELEIA, \u06DD,
    ARABIC END OF AYAH and \u06DE, ARABIC START OF RUB EL HIZB. Name the
    second element with the same name except the characters described. The
    document name00802_p.xml sets the elements to 0 and 1 respectively.
    """
    assert_bindings(
...
def test_name00801_name00801_p(mode, save_output, output_format):
    ...
    elements using 7 punctuation characters: hyphen ('-', \u002D, HYPHEN-
    MINUS), period ('.', \u002E, FULL STOP), underscore ('_', \u005F, LOW
    LINE), dot ('.', \u00B7, MIDDLE DOT), \u0387, GREEK ANO TELEIA,
...
def test_name00601m_name00601m1_p(mode, save_output, output_format):
    ...
    declaration. The negative case uses undeclared element.
    """
    assert_bindings(
...
def test_name00201m2_positive(mode, save_output, output_format):
    ...
    the same name. Check that there is no name clash.
    """
    assert_bindings(
...
def test_name00101m_name00101m1_p_293(mode, save_output, output_format):
    """
...
def test_minoccurs00201m_min_occurs00201m1_p(mode, save_output, output_format):
    ...
    global one. Omit minOccurs attribute. Check that the value of the
    {minOccurs} property is 1.
    """
    assert_bindings(
...
def test_minoccurs00101m_min_occurs00101m1_p(mode, save_output, output_format):
    ...
    value of the {minOccurs} property is 1.
    """
    assert_bindings(
...
def test_maxoccurs00201m_max_occurs00201m1_p(mode, save_output, output_format):
    ...
    Omit maxOccurs attribute. Check that the value of the {maxOccurs}
    property is 1.
    """
    assert_bindings(
...
def test_idconstrdefs00501m_id_constr_defs00501m1_p(mode, save_output, output_format):
    ...
    information items may not have that attributes with the same values.
    """
    assert_bindings(
...
def test_idconstrdefs00403m_id_constr_defs00403m1_p(mode, save_output, output_format):
    ...
    of a type derived from IDREFS. Ensure that the second attribute is not
    allowed to refer to a values that are not defined by the first
    attribute.
    """
    assert_bindings(
...
def test_idconstrdefs00402m_id_constr_defs00402m1_p(mode, save_output, output_format):
    ...
    Declare an attribute of a type derived from ID. Declare an attribute
    of a type derived from IDREF. Ensure that the second attribute is not
    allowed to refer to a value that is not defined by the first
    attribute.
    """
    assert_bindings(
...
def test_idconstrdefs00401m_id_constr_defs00401m1_p(mode, save_output, output_format):
    ...
    attribute of a type derived from IDREF. Ensure that the second
    attribute is allowed to refer to a value that is defined by the first
    attribute defined in the same element information item.
    """
    assert_bindings(
...
def test_idconstrdefs00204m_id_constr_defs00204m1_p(mode, save_output, output_format):
    ...
    keyref field is not allowed to refer to a key that is not defined.
    """
    assert_bindings(
...
def test_idconstrdefs00203m_id_constr_defs00203m1_p(mode, save_output, output_format):
    ...
    constraint prevents one pair of values to be defined twice.
    """
    assert_bindings(
...
def test_idconstrdefs00202m_id_constr_defs00202m1_p(mode, save_output, output_format):
    ...
    prevents one key to be defined twice.
    """
    assert_bindings(
...
def test_disallowedsubst00503m2_positive(mode, save_output, output_format):
    ...
    derived by restriction from type of the first element. Prohibiting
    substitutions of the first element's type contain 'extension', so the
...
def test_disallowedsubst00503m1_positive(mode, save_output, output_format):
    ...
    restriction from type of the first element. Prohibiting substitutions
    of the first element's type is empty, so the second element is
...
def test_disallowedsubst00502m2_positive(mode, save_output, output_format):
    ...
    from type of the first element. Blockong constraints of the first
    element contain 'restriction', so the second element is substitutable
    for the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00502m1_positive(mode, save_output, output_format):
    ...
    elements are declared. The second one has type derived by extension
    from type of the first element. Blockong constraints of the first
    element is empty, so the second element is substitutable for the first
    one.
    """
    assert_bindings(
...
def test_disallowedsubst00501m1_positive(mode, save_output, output_format):
    ...
    element. Blockong constraints of the first element is empty, so the
    second element is substitutable for the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00401m1_positive(mode, save_output, output_format):
    ...
    substituteable for another element declaration. In the test any
    substitution for element Head is allowed.
    """
    assert_bindings(
...
def test_disallowedsubst00106m1_positive(mode, save_output, output_format):
    ...
    first element has a type extended from the type of the head. Block
    (disallow) an extension of the head. Try to substitute the head
    element with the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00105m_disallowed_subst00105m1_p(mode, save_output, output_format):
    ...
    type extended from the type of the head. Block (disallow) an extension
    of the head. Try to substitute the head element with the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00102m1_positive(mode, save_output, output_format):
    ...
    element within a substitution group headed by another element. The
    elements have the same type. Block (disallow) a substitution of the
    head. Try to substitute the head element with the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00101m_disallowed_subst00101m1_p(mode, save_output, output_format):
    ...
    same type. Block (disallow) a substitution of the head. Try to
    substitute the head element with the first one.
    """
    assert_bindings(
...
def test_annotation00101m3_positive(mode, save_output, output_format):
    ...
    Annotations provide for human- and machine-targeted annotations of
    schema components. In the test the machine-targeted annotation is
...
def test_annotation00101m1_positive_323(mode, save_output, output_format):
    """
    human-targeted annotation for element declaration (valid schema)
    Annotations provide for human- and machine-targeted annotations of
    schema components. In the test the human-targeted annotation is
    provided for an element declaration.
    """
    assert_bindings(
...
def test_abstract00101m_abstract00101m1_p(mode, save_output, output_format):
    ...
    another element which is abstract. The elements have the same type.
    Try to substitute the head element with the first one. Try to use the
    head itself.
    """
    assert_bindings(
...
def test_targetns00101m1_positive(mode, save_output, output_format):
    ...
    constraint and a keyref to that key inside the element declaration.
    Declare another element with the same key and keyref. Check that if
    the second element declaration is in the same namespace, that the
    schema is invalid, otherwise it is valid.
    """
    assert_bindings(
...
def test_name00201m1_positive_328(mode, save_output, output_format):
    """
    ...
    type and a constraint. Check that there is no name clash.
    """
    assert_bindings(
...
def test_name00101m1_positive(mode, save_output, output_format):
    ...
    a key identity constraint. Define another key identity constraint with
    another name. Check that the schema is valid.
    """
    assert_bindings(
...
def test_fields00203m4_positive(mode, save_output, output_format):
    ...
    appealed to in checking these constraints apply to the value of the
    fields selected, so that for example 3.0 and 3 would be conflicting
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the fields
    have local type string and values "3.0" and "3".
    """
    assert_bindings(
...
def test_fields00203m1_positive(mode, save_output, output_format):
    ...
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the fields
    have local type decimal and values "3.0" and "-3".
    """
    assert_bindings(
...
def test_fields00202m4_positive(mode, save_output, output_format):
    ...
    fields selected, so that for example 3.0 and 3 would be conflicting
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the first
    field of type string has value "3.0". The second field of type string
    has value "3".
    """
    assert_bindings(
...
def test_fields00202m3_positive(mode, save_output, output_format):
    ...
    value of the fields selected, so that for example 3.0 and 3 would be
    conflicting keys if they were both number, but non-conflicting if they
    were both strings, or one was a string and one a number. In the test
    the first field of type string has value "3.0". The second field of
    type decimal has value "3.0".
    """
    assert_bindings(
...
def test_fields00202m2_positive(mode, save_output, output_format):
    ...
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the first
    field of type string has value "3.0". The second field of type decimal
    has value "3".
    """
    assert_bindings(
...
def test_fields00201m4_positive(mode, save_output, output_format):
    ...
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the fields
    of type string have values 3.0 and 3.
    """
    assert_bindings(
...
def test_fields00201m1_positive(mode, save_output, output_format):
    ...
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the fields
    of type decimal have values 3.0 and -3.
    """
    assert_bindings(
...
def test_annotation00101m4_positive_339(mode, save_output, output_format):
    """
    ...
    annotations of schema components. In the test the machine-targeted
    annotation is provided for an Identity-constraint Definition.
    """
    assert_bindings(
...
def test_annotation00101m1_positive_340(mode, save_output, output_format):
    """
...
def test_annotation00101m7_positive(mode, save_output, output_format):
    ...
    (valid schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the human-targeted
    annotation is provided for a model group schema component (choice).
    """
    assert_bindings(
...
def test_annotation00101m4_positive_360(mode, save_output, output_format):
    """
    machine-targeted annotation for a model group schema component (all)
    (valid schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the machine-targeted
    annotation is provided for a model group schema component (all).
    """
    assert_bindings(
...
def test_annotation00101m16_positive(mode, save_output, output_format):
    ...
    targeted annotations of schema components. In the test the machine-
    targeted annotation is provided for a model group schema component
    (sequence).
    """
    assert_bindings(
...
def test_annotation00101m13_positive(mode, save_output, output_format):
    ...
    (sequence) (valid schema) Annotations provide for human- and machine-
    targeted annotations of schema components. In the test the human-
    targeted annotation is provided for a model group schema component
    (sequence).
    """
    assert_bindings(
...
def test_annotation00101m1_positive_364(mode, save_output, output_format):
    """
...
def test_targetns00101m2_positive(mode, save_output, output_format):
    ...
    {target namespace}; model group identities must be unique within an
    *XML Schema*.
    """
    assert_bindings(
...
def test_targetns00101m1_positive_366(mode, save_output, output_format):
    """
...
def test_name00101m1_positive_367(mode, save_output, output_format):
    """
...
def test_annotation00101m4_positive_371(mode, save_output, output_format):
    """
    machine-targeted annotation for a model group definition (valid
    schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the machine-targeted
    annotation is provided for a model group definition.
    """
    assert_bindings(
...
def test_annotation00101m1_positive_372(mode, save_output, output_format):
    """
...
def test_targetns00101m2_positive_373(mode, save_output, output_format):
    """
    Use of the notation declared in the namespace 'tck_test' (valid
    schema) Notation declarations are referenced in the course of
    *validating* strings as members of the NOTATION simple type.
    """
    assert_bindings(
...
def test_targetns00101m1_positive_374(mode, save_output, output_format):
    """
...
def test_name00101m1_positive_378(mode, save_output, output_format):
    """
    ...
    contain a referebce to undeclared notation.
    """
    assert_bindings(
...
def test_annotation00101m3_positive_379(mode, save_output, output_format):
    """
...
def test_annotation00101m1_positive_380(mode, save_output, output_format):
    """
...
def test_st_variety00101m2_positive(mode, save_output, output_format):
    ...
    must be an atomic simple type definition or a built-in primitive
    datatype. 2. The {final} of the {base type definition} must not
    contain restriction. 3. For each facet in the {facets} there must be a
    facet of the same kind in the {facets} of the {base type definition}
    of whose {value} the facet in question's {value} must be a valid
    restriction as defined in [XML Schemas: Datatypes].
    """
    assert_bindings(
...
def test_st_facets00608_st_facets00608_p(mode, save_output, output_format):
    ...
    NCName and restricted with enumeration values that contain one Unicode
    letter #x01BB which is neither upper nor lower. The document
    ST_facets00608_p.xml uses all the values.
    """
    assert_bindings(
...
def test_st_facets00605_st_facets00605_p(mode, save_output, output_format):
    ...
    with 2 enumeration values "_-." and "_-0.". The document uses each of
    the values.
    """
    assert_bindings(
...
def test_st_facets00604_st_facets00604_p(mode, save_output, output_format):
    ...
    NCName and restricted with several enumeration values that contain
    lower case and upper case letters and non-letter characters: "aaaa",
    "bbbB", "ccCc", "ddDD", "eEee", "fFfF", "pPPp", "gGGG", "Hhhh",
    "IiiI", "JjJj", "KkKK", "LLll", "MMmM", "NNNn", "OOOO", "bbb0",
    "cc0c", "dd00", "e0ee", "f0f0", "p00p", "g000", "bbb_", "cc_c",
    "dd__", "e_ee", "f_f_", "p__p", "g___", "H111", "I11I", "J1J1",
    "K1KK", "LL11", "MM1M", "NNN1", "H---", "I--I", "J-J-", "K-KK",
    "LL--", "MM-M", "NNN-". The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00603_st_facets00603_p(mode, save_output, output_format):
    ...
    (valid schema) Declare a simple type based on NCName and restricted
    with 3 enumeration values "aa111a2Aa", "aa22B3c" and "aa3-4_". The
...
def test_st_facets00602_st_facets00602_p(mode, save_output, output_format):
    ...
    enumeration values
    "a&#x002D;1&#x002E;2&#x005F;3&#x00B7;4&#x0387;5&#x06DD;6&#x06DE;" and
    "a1_2_3_4_5_6". The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00601_st_facets00601_p(mode, save_output, output_format):
    ...
    enumeration values that has the following punctuation characters:
    hyphen ('-', \u002D, HYPHEN-MINUS), period ('.', \u002E, FULL STOP),
    underscore ('_', \u005F, LOW LINE), dot ('.', \u00B7, MIDDLE DOT),
    \u0387, GREEK ANO TELEIA, \u06DD, ARABIC END OF AYAH and \u06DE,
    ARABIC START OF RUB EL HIZB. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00506m2_positive(mode, save_output, output_format):
    ...
    restricted with 3 enumeration values that end with the extender
    characters 0x30fc, 0x30fd, 0x30fe. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00506m1_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 14 enumeration values that end
    with the extender characters 0x00b7, 0x02d0, 0x02d1, 0x0387, 0x0640,
    0x0e46, 0x0ec6, 0x3005, 0x3031, 0x3033, 0x3035, 0x309d, 0x309d,
    0x309e. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00505m9_positive(mode, save_output, output_format):
    ...
    and restricted with 20 enumeration values that end with the combining
    characters 0x0f39, 0x0f3e, 0x0f3f, 0x0f71, 0x0f7a, 0x0f84, 0x0f86,
    0x0f86, 0x0f87, 0x0f90, 0x0f92, 0x0f95, 0x0f97, 0x0f99, 0x0fa3,
    0x0fad, 0x0fb1, 0x0fb4, 0x0fb7, 0x0fb9. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00505m8_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 22 enumeration values that end
    with the combining characters 0x0e31, 0x0e34, 0x0e37, 0x0e3a, 0x0e47,
    0x0e4a, 0x0e4e, 0x0eb1, 0x0eb4, 0x0eb6, 0x0eb9, 0x0ebb, 0x0ebb,
    0x0ebc, 0x0ec8, 0x0eca, 0x0ecd, 0x0f18, 0x0f18, 0x0f19, 0x0f35,
    0x0f37. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00505m7_positive(mode, save_output, output_format):
    ...
    and restricted with 28 enumeration values that end with the combining
    characters 0x0c82, 0x0c82, 0x0c83, 0x0cbe, 0x0cc1, 0x0cc4, 0x0cc6,
    0x0cc7, 0x0cc8, 0x0cca, 0x0ccb, 0x0ccd, 0x0cd5, 0x0cd5, 0x0cd6,
    0x0d02, 0x0d02, 0x0d03, 0x0d3e, 0x0d40, 0x0d43, 0x0d46, 0x0d47,
    0x0d48, 0x0d4a, 0x0d4b, 0x0d4d, 0x0d57. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00505m6_positive(mode, save_output, output_format):
    ...
    0x0bc7, 0x0bc8, 0x0bca, 0x0bcb, 0x0bcd, 0x0bd7, 0x0c01, 0x0c02,
    0x0c03, 0x0c3e, 0x0c41, 0x0c44, 0x0c46, 0x0c47, 0x0c48, 0x0c4a,
    0x0c4b, 0x0c4d, 0x0c55, 0x0c55, 0x0c56. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00505m5_positive(mode, save_output, output_format):
    ...
    enumeration values that end with the combining characters 0x0abc,
    0x0abe, 0x0ac1, 0x0ac5, 0x0ac7, 0x0ac8, 0x0ac9, 0x0acb, 0x0acc,
    0x0acd, 0x0b01, 0x0b02, 0x0b03, 0x0b3c, 0x0b3e, 0x0b40, 0x0b43,
    0x0b47, 0x0b47, 0x0b48, 0x0b4b, 0x0b4c, 0x0b4d, 0x0b56, 0x0b56,
    0x0b57. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00505m4_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 22 enumeration values that end
    with the combining characters 0x09e2, 0x09e2, 0x09e3, 0x0a02, 0x0a3c,
    0x0a3e, 0x0a3f, 0x0a40, 0x0a41, 0x0a42, 0x0a47, 0x0a47, 0x0a48,
    0x0a4b, 0x0a4c, 0x0a4d, 0x0a70, 0x0a70, 0x0a71, 0x0a81, 0x0a82,
    0x0a83. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00505m3_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 22 enumeration values that end
    with the combining characters 0x0951, 0x0952, 0x0954, 0x0962, 0x0962,
    0x0963, 0x0981, 0x0982, 0x0983, 0x09bc, 0x09be, 0x09bf, 0x09c0,
    0x09c2, 0x09c4, 0x09c7, 0x09c7, 0x09c8, 0x09cb, 0x09cc, 0x09cd,
    0x09d7. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00505m2_positive(mode, save_output, output_format):
    ...
    values that end with the combining characters 0x0670, 0x06d6, 0x06d9,
    0x06dc, 0x06dd, 0x06de, 0x06df, 0x06e0, 0x06e2, 0x06e4, 0x06e7,
    0x06e7, 0x06e8, 0x06ea, 0x06eb, 0x06ed, 0x0901, 0x0902, 0x0903,
    0x093c, 0x093e, 0x0945, 0x094c, 0x094d. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00505m10_positive(mode, save_output, output_format):
    ...
    enumeration values that end with the combining characters 0x20d0,
    0x20d6, 0x20dc, 0x20e1, 0x302a, 0x302c, 0x302f, 0x3099, 0x309a. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00505m1_positive(mode, save_output, output_format):
    ...
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that end with the combining characters 0x0300,
    0x0322, 0x0344, 0x0360, 0x0360, 0x0361, 0x0483, 0x0484, 0x0486,
    0x0591, 0x0599, 0x05a1, 0x05a3, 0x05ae, 0x05b9, 0x05bb, 0x05bc,
    0x05bd, 0x05bf, 0x05c1, 0x05c1, 0x05c2, 0x05c4, 0x064b, 0x064e,
    0x0652. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00504m2_positive(mode, save_output, output_format):
    ...
    type based on NCName and restricted with 15 enumeration values that
    end with the digit characters 0x0ce6, 0x0cea, 0x0cef, 0x0d66, 0x0d6a,
    0x0d6f, 0x0e50, 0x0e54, 0x0e59, 0x0ed0, 0x0ed4, 0x0ed9, 0x0f20,
    0x0f24, 0x0f29. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00504m1_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 30 enumeration values that end
    with the digit characters 0x0030, 0x0034, 0x0039, 0x0660, 0x0664,
    0x0669, 0x06f0, 0x06f4, 0x06f9, 0x0966, 0x096a, 0x096f, 0x09e6,
    0x09ea, 0x09ef, 0x0a66, 0x0a6a, 0x0a6f, 0x0ae6, 0x0aea, 0x0aef,
    0x0b66, 0x0b6a, 0x0b6f, 0x0be7, 0x0beb, 0x0bef, 0x0c66, 0x0c6a,
    0x0c6f. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00503m1_positive(mode, save_output, output_format):
    ...
    (valid schema) Declare a simple type based on NCName and restricted
    with 3 enumeration values that end with the characters 0x005f, 0x002e,
    0x002d. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00502m1_positive(mode, save_output, output_format):
    ...
    type based on NCName and restricted with 7 enumeration values that end
    with the ideographic characters 0x4e00, 0x76d2, 0x9fa5, 0x3007,
    0x3021, 0x3025, 0x3029. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m9_positive(mode, save_output, output_format):
    ...
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that end with the basic characters 0x0a8f, 0x0a90,
    0x0a91, 0x0a93, 0x0a9d, 0x0aa8, 0x0aaa, 0x0aad, 0x0ab0, 0x0ab2,
    0x0ab2, 0x0ab3, 0x0ab5, 0x0ab7, 0x0ab9, 0x0abd, 0x0ae0, 0x0b05,
    0x0b08, 0x0b0c, 0x0b0f, 0x0b0f, 0x0b10, 0x0b13, 0x0b1d, 0x0b28. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m8_positive(mode, save_output, output_format):
    ...
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that end with the basic characters 0x0a13, 0x0a1d,
    0x0a28, 0x0a2a, 0x0a2d, 0x0a30, 0x0a32, 0x0a32, 0x0a33, 0x0a35,
    0x0a35, 0x0a36, 0x0a38, 0x0a38, 0x0a39, 0x0a59, 0x0a5a, 0x0a5c,
    0x0a5e, 0x0a72, 0x0a73, 0x0a74, 0x0a85, 0x0a88, 0x0a8b, 0x0a8d. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m7_positive(mode, save_output, output_format):
    ...
    and restricted with 28 enumeration values that end with the basic
    characters 0x098f, 0x098f, 0x0990, 0x0993, 0x099d, 0x09a8, 0x09aa,
    0x09ad, 0x09b0, 0x09b2, 0x09b6, 0x09b7, 0x09b9, 0x09dc, 0x09dc,
    0x09dd, 0x09df, 0x09e0, 0x09e1, 0x09f0, 0x09f0, 0x09f1, 0x0a05,
    0x0a07, 0x0a0a, 0x0a0f, 0x0a0f, 0x0a10. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m6_positive(mode, save_output, output_format):
    ...
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that end with the basic characters 0x0671, 0x0694,
    0x06b7, 0x06ba, 0x06bc, 0x06be, 0x06c0, 0x06c7, 0x06ce, 0x06d0,
    0x06d1, 0x06d3, 0x06d5, 0x06e5, 0x06e5, 0x06e6, 0x0905, 0x091f,
    0x0939, 0x093d, 0x0958, 0x095c, 0x0961, 0x0985, 0x0988, 0x098c. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m5_positive(mode, save_output, output_format):
    ...
    0x04f8, 0x04f9, 0x0531, 0x0543, 0x0556, 0x0559, 0x0561, 0x0573,
    0x0586, 0x05d0, 0x05dd, 0x05ea, 0x05f0, 0x05f1, 0x05f2, 0x0621,
    0x062d, 0x063a, 0x0641, 0x0645, 0x064a. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m4_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 30 enumeration values that end
    with the basic characters 0x03d0, 0x03d3, 0x03d6, 0x03e2, 0x03e9,
    0x03f1, 0x0401, 0x0406, 0x040c, 0x040e, 0x042e, 0x044f, 0x0451,
    0x0456, 0x045c, 0x045e, 0x046f, 0x0481, 0x0490, 0x04a7, 0x04bf,
    0x04c1, 0x04c2, 0x04c4, 0x04c7, 0x04c7, 0x04c8, 0x04cb, 0x04cb,
    0x04cc. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m3_positive(mode, save_output, output_format):
    ...
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that end with the basic characters 0x0276, 0x027a,
    0x027f, 0x0281, 0x0294, 0x02a8, 0x02bb, 0x02be, 0x02c1, 0x0386,
    0x0388, 0x0389, 0x038a, 0x038c, 0x038e, 0x038e, 0x038f, 0x0391,
    0x0399, 0x03a1, 0x03a3, 0x03a9, 0x03af, 0x03b1, 0x03bf, 0x03ce. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m21_positive(mode, save_output, output_format):
    ...
    NCName and restricted with 21 enumeration values that end with the
    basic characters 0x1fe8, 0x1fea, 0x1fec, 0x1ff8, 0x1ff9, 0x1ffb,
    0x2180, 0x2181, 0x2182, 0x3041, 0x306a, 0x3094, 0x30a1, 0x30cd,
    0x30fa, 0x3105, 0x3118, 0x312c, 0xac00, 0xc1d1, 0xd7a3. The document
    uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m20_positive(mode, save_output, output_format):
    ...
    a simple type based on NCName and restricted with 24 enumeration
    values that end with the basic characters 0x1f5b, 0x1f5d, 0x1f5f,
    0x1f6e, 0x1f7d, 0x1fb0, 0x1fb0, 0x1fb1, 0x1fb8, 0x1fb9, 0x1fbb,
    0x1fc8, 0x1fc9, 0x1fcb, 0x1fd0, 0x1fd0, 0x1fd1, 0x1fd8, 0x1fd9,
    0x1fdb, 0x1fe0, 0x1fe0, 0x1fe1, 0x1fe5. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m2_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 30 enumeration values that end
    with the basic characters 0x014a, 0x0164, 0x017e, 0x0180, 0x018a,
    0x0194, 0x0196, 0x019d, 0x01a5, 0x01a7, 0x01a8, 0x01a9, 0x01ab,
    0x01b4, 0x01bd, 0x01c0, 0x01c1, 0x01c3, 0x01cd, 0x01de, 0x01ef,
    0x01f4, 0x01f4, 0x01f5, 0x01fa, 0x0208, 0x0217, 0x0250, 0x0262,
    0x0274. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m19_positive(mode, save_output, output_format):
    ...
    and restricted with 20 enumeration values that end with the basic
    characters 0x1ea0, 0x1ecc, 0x1ef9, 0x1f00, 0x1f0a, 0x1f15, 0x1f18,
    0x1f1a, 0x1f1d, 0x1f20, 0x1f32, 0x1f45, 0x1f48, 0x1f4a, 0x1f4d,
    0x1f51, 0x1f53, 0x1f55, 0x1f57, 0x1f59. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m18_positive(mode, save_output, output_format):
    ...
    enumeration values that end with the basic characters 0x11ab, 0x11ae,
    0x11ae, 0x11af, 0x11b7, 0x11b7, 0x11b8, 0x11ba, 0x11bc, 0x11bf,
    0x11c2, 0x11eb, 0x11f0, 0x11f9, 0x1e00, 0x1e4a, 0x1e95, 0x1e9b. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m17_positive(mode, save_output, output_format):
    ...
    values that end with the basic characters 0x115f, 0x1160, 0x1161,
    0x1163, 0x1165, 0x1167, 0x1169, 0x116d, 0x116d, 0x116e, 0x1172,
    0x1172, 0x1173, 0x1175, 0x119e, 0x11a8. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m16_positive(mode, save_output, output_format):
    ...
    a simple type based on NCName and restricted with 16 enumeration
    values that end with the basic characters 0x110b, 0x110b, 0x110c,
    0x110e, 0x1110, 0x1112, 0x113c, 0x113e, 0x1140, 0x114c, 0x114e,
    0x1150, 0x1154, 0x1154, 0x1155, 0x1159. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m15_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 22 enumeration values that end
    with the basic characters 0x0eb0, 0x0eb2, 0x0eb2, 0x0eb3, 0x0ebd,
    0x0ec0, 0x0ec2, 0x0ec4, 0x0f40, 0x0f43, 0x0f47, 0x0f49, 0x0f59,
    0x0f69, 0x1100, 0x1102, 0x1102, 0x1103, 0x1105, 0x1106, 0x1107,
    0x1109. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m14_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 22 enumeration values that end
    with the basic characters 0x0e87, 0x0e87, 0x0e88, 0x0e8a, 0x0e8d,
    0x0e94, 0x0e95, 0x0e97, 0x0e99, 0x0e9c, 0x0e9f, 0x0ea1, 0x0ea2,
    0x0ea3, 0x0ea5, 0x0ea7, 0x0eaa, 0x0eaa, 0x0eab, 0x0ead, 0x0ead,
    0x0eae. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m13_positive(mode, save_output, output_format):
    ...
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that end with the basic characters 0x0d0e, 0x0d0f,
    0x0d10, 0x0d12, 0x0d1d, 0x0d28, 0x0d2a, 0x0d31, 0x0d39, 0x0d60,
    0x0d60, 0x0d61, 0x0e01, 0x0e17, 0x0e2e, 0x0e30, 0x0e32, 0x0e32,
    0x0e33, 0x0e40, 0x0e42, 0x0e45, 0x0e81, 0x0e81, 0x0e82, 0x0e84. The
    document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00501m12_positive(mode, save_output, output_format):
    ...
    0x0c88, 0x0c8c, 0x0c8e, 0x0c8f, 0x0c90, 0x0c92, 0x0c9d, 0x0ca8,
    0x0caa, 0x0cae, 0x0cb3, 0x0cb5, 0x0cb7, 0x0cb9, 0x0cde, 0x0ce0,
    0x0ce0, 0x0ce1, 0x0d05, 0x0d08, 0x0d0c. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m11_positive(mode, save_output, output_format):
    ...
    0x0ba8, 0x0ba9, 0x0baa, 0x0bae, 0x0bb1, 0x0bb5, 0x0bb7, 0x0bb8,
    0x0bb9, 0x0c05, 0x0c08, 0x0c0c, 0x0c0e, 0x0c0f, 0x0c10, 0x0c12,
    0x0c1d, 0x0c28, 0x0c2a, 0x0c2e, 0x0c33. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m10_positive(mode, save_output, output_format):
    ...
    0x0b37, 0x0b39, 0x0b3d, 0x0b5c, 0x0b5c, 0x0b5d, 0x0b5f, 0x0b60,
    0x0b61, 0x0b85, 0x0b87, 0x0b8a, 0x0b8e, 0x0b8f, 0x0b90, 0x0b92,
    0x0b93, 0x0b95, 0x0b99, 0x0b99, 0x0b9a. The document uses each of the
    values.
    """
    assert_bindings(
...
def test_st_facets00501m1_positive(mode, save_output, output_format):
    ...
    based on NCName and restricted with 30 enumeration values that end
    with the basic characters 0x0041, 0x004d, 0x005a, 0x0061, 0x0064,
    0x0068, 0x006a, 0x0072, 0x007a, 0x00c0, 0x00cb, 0x00d6, 0x00d8,
    0x00db, 0x00de, 0x00e0, 0x00eb, 0x00f6, 0x00f8, 0x00fb, 0x00ff,
    0x0100, 0x0118, 0x0131, 0x0134, 0x0139, 0x013e, 0x0141, 0x0144,
    0x0148. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00403m1_positive(mode, save_output, output_format):
    ...
    enumeration value that begins with the underscore character 0x005f.
    The document uses the value.
    """
    assert_bindings(
...
def test_st_facets00401m9_positive(mode, save_output, output_format):
    ...
    0x0b08, 0x0b0c, 0x0b0f, 0x0b0f, 0x0b10, 0x0b13, 0x0b1d, 0x0b28 (valid
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that begin with the basic characters 0x0a8f,
    0x0a90, 0x0a91, 0x0a93, 0x0a9d, 0x0aa8, 0x0aaa, 0x0aad, 0x0ab0,
    0x0ab2, 0x0ab2, 0x0ab3, 0x0ab5, 0x0ab7, 0x0ab9, 0x0abd, 0x0ae0,
    0x0b05, 0x0b08, 0x0b0c, 0x0b0f, 0x0b0f, 0x0b10, 0x0b13, 0x0b1d,
    0x0b28. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00401m8_positive(mode, save_output, output_format):
    ...
    0x0a5e, 0x0a72, 0x0a73, 0x0a74, 0x0a85, 0x0a88, 0x0a8b, 0x0a8d (valid
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that begin with the basic characters 0x0a13,
    0x0a1d, 0x0a28, 0x0a2a, 0x0a2d, 0x0a30, 0x0a32, 0x0a32, 0x0a33,
    0x0a35, 0x0a35, 0x0a36, 0x0a38, 0x0a38, 0x0a39, 0x0a59, 0x0a5a,
    0x0a5c, 0x0a5e, 0x0a72, 0x0a73, 0x0a74, 0x0a85, 0x0a88, 0x0a8b,
    0x0a8d. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00401m7_positive(mode, save_output, output_format):
    ...
    0x09e1, 0x09f0, 0x09f0, 0x09f1, 0x0a05, 0x0a07, 0x0a0a, 0x0a0f,
    0x0a0f, 0x0a10 (valid schema) Declare a simple type based on NCName
    and restricted with 28 enumeration values that begin with the basic
    characters 0x098f, 0x098f, 0x0990, 0x0993, 0x099d, 0x09a8, 0x09aa,
    0x09ad, 0x09b0, 0x09b2, 0x09b6, 0x09b7, 0x09b9, 0x09dc, 0x09dc,
    0x09dd, 0x09df, 0x09e0, 0x09e1, 0x09f0, 0x09f0, 0x09f1, 0x0a05,
    0x0a07, 0x0a0a, 0x0a0f, 0x0a0f, 0x0a10. The document uses each of the
...
def test_st_facets00401m6_positive(mode, save_output, output_format):
    ...
    0x0939, 0x093d, 0x0958, 0x095c, 0x0961, 0x0985, 0x0988, 0x098c (valid
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that begin with the basic characters 0x0671,
    0x0694, 0x06b7, 0x06ba, 0x06bc, 0x06be, 0x06c0, 0x06c7, 0x06ce,
    0x06d0, 0x06d1, 0x06d3, 0x06d5, 0x06e5, 0x06e5, 0x06e6, 0x0905,
    0x091f, 0x0939, 0x093d, 0x0958, 0x095c, 0x0961, 0x0985, 0x0988,
    0x098c. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00401m4_positive(mode, save_output, output_format):
    ...
    0x0490, 0x04a7, 0x04bf, 0x04c1, 0x04c2, 0x04c4, 0x04c7, 0x04c7,
    0x04c8, 0x04cb, 0x04cb, 0x04cc (valid schema) Declare a simple type
    based on NCName and restricted with 30 enumeration values that begin
    with the basic characters 0x03d0, 0x03d3, 0x03d6, 0x03e2, 0x03e9,
    0x03f1, 0x0401, 0x0406, 0x040c, 0x040e, 0x042e, 0x044f, 0x0451,
    0x0456, 0x045c, 0x045e, 0x046f, 0x0481, 0x0490, 0x04a7, 0x04bf,
    0x04c1, 0x04c2, 0x04c4, 0x04c7, 0x04c7, 0x04c8, 0x04cb, 0x04cb,
...
def test_st_facets00401m3_positive(mode, save_output, output_format):
    ...
    0x0399, 0x03a1, 0x03a3, 0x03a9, 0x03af, 0x03b1, 0x03bf, 0x03ce (valid
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that begin with the basic characters 0x0276,
    0x027a, 0x027f, 0x0281, 0x0294, 0x02a8, 0x02bb, 0x02be, 0x02c1,
    0x0386, 0x0388, 0x0389, 0x038a, 0x038c, 0x038e, 0x038e, 0x038f,
    0x0391, 0x0399, 0x03a1, 0x03a3, 0x03a9, 0x03af, 0x03b1, 0x03bf,
    0x03ce. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00401m21_positive(mode, save_output, output_format):
    ...
    0x306a, 0x3094, 0x30a1, 0x30cd, 0x30fa, 0x3105, 0x3118, 0x312c,
    0xac00, 0xc1d1, 0xd7a3 (valid schema) Declare a simple type based on
    NCName and restricted with 21 enumeration values that begin with the
    basic characters 0x1fe8, 0x1fea, 0x1fec, 0x1ff8, 0x1ff9, 0x1ffb,
    0x2180, 0x2181, 0x2182, 0x3041, 0x306a, 0x3094, 0x30a1, 0x30cd,
    0x30fa, 0x3105, 0x3118, 0x312c, 0xac00, 0xc1d1, 0xd7a3. The document
    uses each of the values.
...
def test_st_facets00401m20_positive(mode, save_output, output_format):
    ...
    0x1fd9, 0x1fdb, 0x1fe0, 0x1fe0, 0x1fe1, 0x1fe5 (valid schema) Declare
    a simple type based on NCName and restricted with 24 enumeration
    values that begin with the basic characters 0x1f5b, 0x1f5d, 0x1f5f,
    0x1f6e, 0x1f7d, 0x1fb0, 0x1fb0, 0x1fb1, 0x1fb8, 0x1fb9, 0x1fbb,
    0x1fc8, 0x1fc9, 0x1fcb, 0x1fd0, 0x1fd0, 0x1fd1, 0x1fd8, 0x1fd9,
    0x1fdb, 0x1fe0, 0x1fe0, 0x1fe1, 0x1fe5. The document uses each of the
    values.
...
def test_st_facets00401m19_positive(mode, save_output, output_format):
    ...
    0x1f32, 0x1f45, 0x1f48, 0x1f4a, 0x1f4d, 0x1f51, 0x1f53, 0x1f55,
    0x1f57, 0x1f59 (valid schema) Declare a simple type based on NCName
    and restricted with 20 enumeration values that begin with the basic
    characters 0x1ea0, 0x1ecc, 0x1ef9, 0x1f00, 0x1f0a, 0x1f15, 0x1f18,
    0x1f1a, 0x1f1d, 0x1f20, 0x1f32, 0x1f45, 0x1f48, 0x1f4a, 0x1f4d,
    0x1f51, 0x1f53, 0x1f55, 0x1f57, 0x1f59. The document uses each of the
    values.
...
def test_st_facets00401m18_positive(mode, save_output, output_format):
    ...
    0x11c2, 0x11eb, 0x11f0, 0x11f9, 0x1e00, 0x1e4a, 0x1e95, 0x1e9b (valid
    schema) Declare a simple type based on NCName and restricted with 18
    enumeration values that begin with the basic characters 0x11ab,
    0x11ae, 0x11ae, 0x11af, 0x11b7, 0x11b7, 0x11b8, 0x11ba, 0x11bc,
    0x11bf, 0x11c2, 0x11eb, 0x11f0, 0x11f9, 0x1e00, 0x1e4a, 0x1e95,
    0x1e9b. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_facets00401m17_positive(mode, save_output, output_format):
    ...
    0x1172, 0x1172, 0x1173, 0x1175, 0x119e, 0x11a8 (valid schema) Declare
    a simple type based on NCName and restricted with 16 enumeration
    values that begin with the basic characters 0x115f, 0x1160, 0x1161,
    0x1163, 0x1165, 0x1167, 0x1169, 0x116d, 0x116d, 0x116e, 0x1172,
    0x1172, 0x1173, 0x1175, 0x119e, 0x11a8. The document uses each of the
    values.
    """
...
def test_st_facets00401m16_positive(mode, save_output, output_format):
    ...
    0x114e, 0x1150, 0x1154, 0x1154, 0x1155, 0x1159 (valid schema) Declare
    a simple type based on NCName and restricted with 16 enumeration
    values that begin with the basic characters 0x110b, 0x110b, 0x110c,
    0x110e, 0x1110, 0x1112, 0x113c, 0x113e, 0x1140, 0x114c, 0x114e,
    0x1150, 0x1154, 0x1154, 0x1155, 0x1159. The document uses each of the
    values.
    """
...
def test_st_facets00401m15_positive(mode, save_output, output_format):
    ...
    0x0f47, 0x0f49, 0x0f59, 0x0f69, 0x1100, 0x1102, 0x1102, 0x1103,
    0x1105, 0x1106, 0x1107, 0x1109 (valid schema) Declare a simple type
    based on NCName and restricted with 22 enumeration values that begin
    with the basic characters 0x0eb0, 0x0eb2, 0x0eb2, 0x0eb3, 0x0ebd,
    0x0ec0, 0x0ec2, 0x0ec4, 0x0f40, 0x0f43, 0x0f47, 0x0f49, 0x0f59,
    0x0f69, 0x1100, 0x1102, 0x1102, 0x1103, 0x1105, 0x1106, 0x1107,
    0x1109. The document uses each of the values.
...
def test_st_facets00401m13_positive(mode, save_output, output_format):
    ...
    0x0e33, 0x0e40, 0x0e42, 0x0e45, 0x0e81, 0x0e81, 0x0e82, 0x0e84 (valid
    schema) Declare a simple type based on NCName and restricted with 26
    enumeration values that begin with the basic characters 0x0d0e,
    0x0d0f, 0x0d10, 0x0d12, 0x0d1d, 0x0d28, 0x0d2a, 0x0d31, 0x0d39,
    0x0d60, 0x0d60, 0x0d61, 0x0e01, 0x0e17, 0x0e2e, 0x0e30, 0x0e32,
    0x0e32, 0x0e33, 0x0e40, 0x0e42, 0x0e45, 0x0e81, 0x0e81, 0x0e82,
    0x0e84. The document uses each of the values.
    """
    assert_bindings(
...
def test_st_basetd00201m_st_base_td00201m1_p(mode, save_output, output_format):
    ...
    absent the type definition corresponding to the 'simpleType' among the
...
def test_st_basetd00101m_st_base_td00101m1_p(mode, save_output, output_format):
    ...
    type definition *resolved* to by the *actual value* of the base
...
def test_annotations00101m6_positive(mode, save_output, output_format):
    ...
    (valid schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the machine-targeted
    annotation is provided for the schema itself. The annotation is placed
    at the end.
    """
    assert_bindings(
...
def test_annotations00101m5_positive(mode, save_output, output_format):
    ...
    schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the machine-targeted
    annotation is provided for the schema itself. The annotation is
    specified twice.
    """
    assert_bindings(
...
def test_annotations00101m2_positive(mode, save_output, output_format):
    ...
    Annotations provide for human- and machine-targeted annotations of
    schema components. In the test the human-targeted annotation is
    provided for the schema itself. The annotation is specified twice.
    """
    assert_bindings(
...
def test_pscontents00302m2_positive(mode, save_output, output_format):
    ...
    information items allowed by wildcards, as follows: lax If the item,
    or any items among its [children] if it's an element information item,
    has a uniquely determined declaration available, it must be *valid*
    with respect to that definition, that is, *validate* where you can,
...
def test_pscontents00202m1_positive(mode, save_output, output_format):
    ...
    as follows: skip No constraints at all: the item must simply be well-
    formed XML.
    """
    assert_bindings(
...
def test_pscontents00102m1_positive(mode, save_output, output_format):
    ...
    schema) {process contents} controls the impact on *assessment* of the
    information items allowed by wildcards, as follows: strict There must
    be a top-level declaration for the item available, or the item must
    have an xsi:type, and the item must be *valid* as appropriate.
    """
    assert_bindings(
...
def test_nsconstraint00302m1_positive(mode, save_output, output_format):
    ...
    set whose members are either namespace names or *absent*) have any of
    the specified namespaces and/or, if *absent* is included in the set,
    are unqualified.
    """
    assert_bindings(
...
def test_nsconstraint00202m1_positive(mode, save_output, output_format):
    ...
    *validation* of attribute and element items that: 2. (not and a
    namespace name) have any namespace other than the specified namespace
    name;
    """
    assert_bindings(
...
def test_nsconstraint00102m2_positive(mode, save_output, output_format):
    ...
    that: 1. (any) have any namespace or are not namespace qualified;
    """
    assert_bindings(
...
def test_nsconstraint00102m1_positive(mode, save_output, output_format):
    ...
    *validation* of attribute and element items that: 1. (any) have any
    namespace or are not namespace qualified;
    """
    assert_bindings(
...
def test_annotation00101m7_positive_539(mode, save_output, output_format):
    """
    machine-targeted annotation for a wildcard schema component (any)
    (valid schema) Annotations provide for human- and machine-targeted
    annotations of schema components. In the test the machine-targeted
    annotation is provided for a wildcard schema component (any).
    """
    assert_bindings(
...
def test_annotation00101m3_positive_541(mode, save_output, output_format):
    """
    machine-targeted annotation for a wildcard schema component
    (anyAttribute) (valid schema) Annotations provide for human- and
    machine-targeted annotations of schema components. In the test the
    machine-targeted annotation is provided for a wildcard schema
    component (anyAttribute).
    """
    assert_bindings(
...
def test_annotation00101m1_positive_542(mode, save_output, output_format):
    """
    human-targeted annotation for a wildcard schema component
    (anyAttribute) (valid schema) Annotations provide for human- and
    machine-targeted annotations of schema components. In the test the
    human-targeted annotation is provided for a wildcard schema component
    (anyAttribute).
    """
    assert_bindings(
