...
def test_valueconstraint01101m4_negative(mode, save_output):
    ...
    if the type definition is a simple type definition, then the string
    must be valid with respect to that definition. In the test the default
    value is invalid for the local type definition.
    """
    assert_bindings(
...
def test_valueconstraint01101m2_negative(mode, save_output):
    ...
    For a string to be a valid default with respect to a type definition,
    if the type definition is a simple type definition, then the string
    must be valid with respect to that definition. In the test the fixed
    value is invalid for the local type definition.
    """
    assert_bindings(
...
def test_valueconstraint00501m6_negative(mode, save_output):
    ...
    element nor character [children] and xsi:nil is not specified, the
    actual type definition is a local type definition then the lexical
    representation of the {value constraint} value must be a valid default
    for the actual type definition . In the test the default value is
    invalid for the local type definition.
    """
    assert_bindings(
...
def test_valueconstraint00501m3_negative(mode, save_output):
    ...
    nor character [children] and xsi:nil is not specified, the actual type
    definition is a local type definition then the lexical representation
    of the {value constraint} value must be a valid default for the actual
    type definition . In the test the fixed value is invalid for the local
    type definition.
    """
    assert_bindings(
...
def test_typedef00802m2_negative(mode, save_output):
    ...
    {type definition} given the {disallowed substitutions}. In the test
    the local type is 'dissalowed'.
    """
    assert_bindings(
...
def test_typedef00801m3_negative(mode, save_output):
    ...
    and the {type definition}'s {prohibited substitutions}. In the test
    the local type is 'dissalowed'.
    """
    assert_bindings(
...
def test_disallowedsubst00503m5_negative(mode, save_output):
    ...
    substitutions of the first element's type contain '#all', so the
    second element is not substitutable for the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00503m4_negative(mode, save_output):
    ...
    element. Prohibiting substitutions of the first element's type contain
    'restriction extension', so the second element is not substitutable
    for the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00502m4_negative(mode, save_output):
    ...
    (valid schema) Two elements are declared. The second one has type
    derived by extension from type of the first element. Blockong
    constraints of the first element contain 'restriction extension', so
    the second element is not substitutable for the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00502m3_negative(mode, save_output):
    ...
    element contain 'extension', so the second element is not
    substitutable for the first one.
    """
    assert_bindings(
...
def test_disallowedsubst00501m2_negative(mode, save_output):
    ...
    second one has type derived by restriction from type of the first
    element. Blockong constraints of the first element contain
    'restriction', so the second element is not substitutable for the
    first one.
    """
    assert_bindings(
...
def test_disallowedsubst00401m2_negative(mode, save_output):
    ...
    substitution for element Head is disallowed.
    """
    assert_bindings(
...
def test_fields00203m5_negative(mode, save_output):
    ...
    have local type string and values "3.0" and "3.0".
    """
    assert_bindings(
...
def test_fields00203m3_negative(mode, save_output):
    ...
    the fields have local type decimal and values "3.0" and "3.0".
    """
    assert_bindings(
...
def test_fields00201m5_negative(mode, save_output):
    ...
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the fields
    of type string have values 3.0 and 3.0.
    """
    assert_bindings(
...
def test_fields00201m3_negative(mode, save_output):
    ...
    keys if they were both number, but non-conflicting if they were both
    strings, or one was a string and one a number. In the test the fields
    of type decimal have values 3.0 and 3.0.
    """
    assert_bindings(
